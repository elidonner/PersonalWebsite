import { Practice, Computer, Versus } from "./modules/modes.js";
import { EMPTY_HAND, GameModeDescription, Difficulty, } from "./types.js";
/**
...
let round_info;
let steps = [];
let first_card = null; // variable to remember the first card that was selected so operation is done in correct order
//EVENT HANDLERS
/**
 ...
 */
function get_and_set_game_mode() {
 ...
 */
function set_game_mode(game_mode) {
    switch (game_mode) {
...
function create_solution_dom() {
    ...
    for (let i = 0; i < solution_steps.length; i++) {
        ...
        var solutionOperation = solution_steps[i].getElementsByClassName("solutionOperation")[0];
        // The first card we need to calculate
        if (i > 0) {
            ans = map_ops(ans, round_info.solution.organized_cards[i], round_info.solution.operations[i - 1]);
        }
 ...
 */
function operation_selected(operation) {
 ...
 */
function clear_selected_operations() {
    for (let i = 0; i < operation_buttons.length; i++) {
